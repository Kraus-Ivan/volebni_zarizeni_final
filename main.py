...
def vyhodnoceni_hlasu():
    ...
    hlasy.reverse() #reversne list s hlasy, aby zaznamenal jen ty posledni hlasy
    list_ser_cisel: List[number] = [] # list s dosud zaznamenanymi seriovymi cisly

    for list_s_hlasem in hlasy:
    ...
    for i in pocet_hlasu:
        if i > 0:
            basic.show_string(String.from_char_code(pozice_hlasu+65))
            basic.show_number(i)
        pozice_hlasu += 1
